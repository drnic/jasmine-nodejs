// By Andrew Cantino (Iteration Labs, LLC) and David Stevenson

var fs = require("fs"),
    path = require("path"),
    sys = require("sys");

var jasminePath = path.join(path.dirname(__filename), "..", "..", "lib", "jasmine-0.10.1.js");
sys.puts("Loading jasmine from " + jasminePath);

var window = GLOBAL;
eval(fs.readFileSync(jasminePath));
...
//  for(k in opt_global) {
//    jasmineKeys.push(k);
//    jasmineValues.push(jasmine[k]);
//  }
//  fileData =  fs.readFileSync(url);
//  fileData = "function testotron(" + jasmineKeys.join(",") + ") { "+fileData+" }";
//  sys.puts(fileData)
//  process.compile(fileData, url);
//  testotron.apply(GLOBAL, [jasmineValues]);
//  eval(fs.readFileSync(url));
  var data = fs.readFileSync(url);
//  sys.puts(data);
  eval(data);
//  process.evalcx(data);
//  process.compileUnsafe(data, url);
};
...
exports.runDefaultNodeSpecs = function() {
  exports.include("spec/spec_helper.js");
